  ...
  return (
    <div className="min-h-screen bg-[#F5F0E8] relative z-50">
      ...
      <div className="bg-[#6C63FF] px-5 pt-14 pb-5 flex items-center justify-between flex-shrink-0">
        ...
        <button
          onClick={sendToGroup}
          disabled={completedCount === 0 || sending}
          className={`text-sm font-black px-4 py-2 rounded-full transition-all ${
            completedCount > 0 && !sending
              ? 'bg-white text-[#6C63FF] shadow-md'
              : 'text-white/30'
          }`}
        >
          {sending ? 'Sending...' : `Send ${completedCount > 0 ? `(${completedCount})` : ''}`}
        </button>
      </div>
          ...
          >
            ...
            <button
              ...
              className={`w-6 h-6 rounded-full border-2 flex-shrink-0 flex items-center justify-center transition-all ${
                task.completed ? 'bg-[#6C63FF] border-[#6C63FF]' : 'border-gray-300'
              }`}
            >
      ...
      {completedCount > 0 && (
        <div className="fixed bottom-8 left-0 right-0 flex justify-center px-6">
          ...
          >
            {sending ? '⏳ Sending...' : `📤 Send ${completedCount} completed task${completedCount > 1 ? 's' : ''} to group`}
          </button>
        </div>
      )}
